...
const startSwapApp = async (
  ...
  servewallet = new ServeWallet(page, servewalletPort, frontendPort, host, testName, projectName, { simulator: true });
  ...
  simulatorProc = startSimulator(simulatorPath, testName, projectName, true);
...
test('Swap defaults to ethereum to bitcoin and disables same-coin selections', async ({
  ...
  await test.step('Start app with simulator wallet', async () => {
    await startSwapApp(page, host, frontendPort, servewalletPort, testInfo.title, testInfo.project.name);
  });
