...
export const App = () => {
  ...
  const maybeRoute = useCallback(() => {
    ...
    if (
      ...
      !accounts.some((account) =>
        currentURL.startsWith('/account/' + account.code),
      )
    ) {
      navigate('/');
      return;
  ...
  const showBottomNavigation =
    deviceIDs.length > 0 || activeAccounts.length > 0;
  ...
  return (
    <ConnectedApp>
      <Providers>
        ...
        <div className="app">
          <AuthRequired />
          <Sidebar accounts={activeAccounts} devices={devices} />
          <div
            className={`${styles.appContent} ${showBottomNavigation ? styles.hasBottomNavigation : ''}`}
          >
            <WCSigningRequest />
            <Aopp />
              ...
              if (productName === 'bitbox02') {
                return (
                  <Fragment key={deviceID}>
                    <BitBox02Wizard deviceID={deviceID} />
                  </Fragment>
                );
              }
              return null;
