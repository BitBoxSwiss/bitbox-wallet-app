...
export const SetDeviceName = ({
  ...
  const { error, invalidChars, nameIsTooShort } =
    useValidateDeviceName(deviceName);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ViewButtons>
          <Button disabled={!!error} primary type="submit">
            {t('button.continue')}
          </Button>
...
export const DeviceNameErrorMessage = ({
  error,
  invalidChars,
}: TDeviceNameErrorMessageProps) => {
  const { t } = useTranslation();
  if (error === 'tooShort') {
    return null;
  }
