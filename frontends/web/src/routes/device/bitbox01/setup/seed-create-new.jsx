...
class SeedCreateNew extends Component {
  ...
  validate = () => {
    if (
      !this.walletNameInput.current ||
      !this.walletNameInput.current.validity.valid ||
      !this.validAgreements()
    ) {
      return false;
    }
  ...
  validAgreements = () => {
    ...
    const invalid = Object.keys(agreements)
      .map((agr) => agreements[agr])
      .includes(false);
    return !invalid;
  };
  ...
  render() {
    ...
    const { showInfo, status, walletName, error, agreements } = this.state;
