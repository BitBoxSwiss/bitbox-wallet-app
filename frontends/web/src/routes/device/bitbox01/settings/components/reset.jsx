...
class Reset extends Component {
  ...
  resetDevice = () => {
    ...
    apiPost('devices/' + this.props.deviceID + '/reset', {
      pin: this.state.pin,
    }).then((data) => {
      this.abort();
      if (data.success) {
        ...
        alertUser(
          this.props.t(`bitbox.error.e${data.code}`, {
  ...
  render() {
    const { t } = this.props;
    const { isConfirming, activeDialog, understand, pin } = this.state;
    return (
      <div>
        <SettingsButton
          danger
        ...
        {activeDialog && (
          <DialogLegacy title={t('reset.title')} onClose={this.abort}>
        ...
        {isConfirming ? <WaitDialog title={t('reset.title')} /> : null}
      </div>
    );
  }
}
