...
class MobilePairing extends Component<Props, State> {
  ...
  private startPairing = () => {
    confirmation(this.props.t('pairing.confirm'), (response) => {
      ...
      apiPost('devices/' + this.props.deviceID + '/pairing/start').then(
        (channel) => {
  ...
  public render() {
    ...
    if (status === 'start') {
      content = (
        <div>
          <div className="flex flex-row flex-start">
            <div>
              <p className="m-top-none">
                <strong className="m-right-quarter">1.</strong>{' '}
                {t('pairing.start.step1')}
              </p>
              <p>
                <Button
                  primary
                ...
                >
                  {t(
                    `pairing.start.${showQRCode ? 'hideAppQRCode' : 'revealAppQRCode'}`,
                  )}
                </Button>
              </p>
              {showQRCode ? (
                  ...
                  <div className="columns">
                    <div className="column column-1-2">
                      ...
                      <div className="flex flex-column flex-center flex-items-center">
                        <QRCode
                          tapToCopy={false}
                          data="https://itunes.apple.com/us/app/digital-bitbox-2fa/id1079896740"
                          size={148}
                        />
                    ...
                    <div className="column column-1-2">
                      ...
                      <div className="flex flex-column flex-center flex-items-center">
                        <QRCode
                          tapToCopy={false}
                          data="https://play.google.com/store/apps/details?id=com.digitalbitbox.tfa"
                          size={148}
                        />
          ...
          <div className="flex flex-row flex-start m-top-default">
            <div>
              <p className="m-top-none">
                <strong className="m-right-quarter">2.</strong>
                {t('pairing.start.step2')}
              </p>
      ...
      content = (
        <QRCode
          tapToCopy={false}
          data={JSON.stringify({ channel, connectOnly: true })}
        />
      );
    } else if (status) {
    ...
    return (
      <div>
        <SettingsButton
          onClick={
            hasMobileChannel && !paired
              ? this.reconnectUnpaired
              : this.startPairing
          }
          optionalText={t(
            `deviceSettings.pairing.status.${paired ? 'true' : 'false'}`,
          )}
        >
          {deviceLocked
            ? hasMobileChannel
              ? t('pairing.reconnectOnly.button')
              : t('pairing.connectOnly.button')
            : hasMobileChannel && !paired
              ? t('pairing.reconnectOnly.button')
              : t('pairing.button')}
        </SettingsButton>
        {status && (
          <DialogLegacy title={t('pairing.title')} onClose={this.abort} medium>
            <div className="flex flex-column flex-center flex-items-center">
              {channel ? content : <p>{t('loading')}</p>}
            </div>
            <DialogButtons>
