...
export const isBitcoinCoin = (coin: CoinUnit) =>
  coin === 'BTC' || coin === 'TBTC' || coin === 'sat' || coin === 'tsat';
...
export const getAccountsByKeystore = (
  accounts: IAccount[],
): TAccountsByKeystore[] => {
...
export const isAmbiguousName = (
  ...
  return (
    keystoreNames.filter((keystore) => keystore.keystore.name === name).length >
    1
  );
};
...
export const getAccountsPerCoin = (accounts: IAccount[]): TAccountCoinMap => {
  return accounts.reduce<Partial<TAccountCoinMap>>(
    (accountPerCoin, account) => {
