...
const renderPercentageDiff = (difference: number | null | undefined, onClick = vi.fn()) => {
  render(
...
describe('PercentageDiff', () => {
  it('does not render an invisible toggle while loading', () => {
    renderPercentageDiff(undefined);
  ...
  it('renders no placeholder when performance is unavailable and keeps the toggle usable', () => {
    ...
    expect(toggle.querySelector('img')).toBeVisible();
    fireEvent.click(toggle);
    expect(onClick).toHaveBeenCalledOnce();
  });
});
