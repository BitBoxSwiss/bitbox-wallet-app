...
export const summedFiatAmount = (
  ...
  unit: ConversionUnit,
): TSummedFiatAmount | undefined => {
  let sum = 0;
  let estimated = false;
  for (const transaction of transactions) {
    const amount = amountForChartTransaction(transaction);
    const conversion = amount.conversions?.[unit];
    if (!conversion) {
      return undefined;
    }
    ...
    sum += parsedAmount;
    estimated = estimated || amount.estimated;
  }
  const formattedSum = sum.toFixed(fiatDecimals(unit));
  return {
    amount: unit === 'sat' ? formattedSum : formatGroupedAmount(formattedSum),
    estimated,
  };
};
...
export const Chart = ({
  ...
  const cacheChartData = useCallback((chartData: ChartData) => {
  ...
  const setChartMarkers = useCallback((nextSource: 'daily' | 'hourly', markerSizeScale = getMarkerSizeScale()) => {
    ...
    const nextMarkers = buildChartMarkers(data.chartTransactions || [], nextSource, chartData, {
      mixed: rootStyle.getPropertyValue('--color-mediumgray-alt').trim(),
  ...
  }, [cacheChartData, calculateChange, chartDisplay, data.chartDataDaily, data.chartDataHourly, data.chartDataMissing, data.chartFiat, handleChartClick, handleCrosshair, hasData, hideAmounts, i18n.language, isDarkMode, isMobile, setChartMarkers]);

  useEffect(() => {
    const chartData = source === 'daily' ? data.chartDataDaily : data.chartDataHourly;
    const previousChartData = source === 'daily' ? prevChartDataDaily : prevChartDataHourly;
    if (!lineSeries.current || previousChartData === chartData) {
      return;
    }
  ...
  useEffect(() => {
    if (prevChartFiat === undefined || prevChartFiat === data.chartFiat) {
      return;
    }
    removeChart();
    initChart();
  }, [data.chartFiat, initChart, prevChartFiat, removeChart]);

  useEffect(() => {
    if (prevHideAmounts === undefined || prevHideAmounts === hideAmounts) {
      return;
    }
    chart.current?.applyOptions({
      leftPriceScale: {
  ...
  }, [hideAmounts, isMobile, prevHideAmounts]);
    ...
    return (
      <span key={type} className={styles.markerTooltipRow}>
        ...
        <span className={styles.markerTooltipAmount}>
          {fiatAmount?.estimated ? '\u2248 ' : ''}
          {fiatAmount !== undefined ? txSign(type) : ''}
          <Amount amount={fiatAmount?.amount || ''} unit={chartFiat} />
