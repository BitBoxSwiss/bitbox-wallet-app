...
const satAmount = (value: string): coinsApi.TBtcSatAmount => ({
  amount: value,
  unit: 'sat',
  estimated: false,
});
...
describe('LightningTopUp', () => {
  ...
  it('revalidates the balance limit before sending the top-up', async () => {
    ...
    const getBalance = vi.spyOn(lightningApi, 'getLightningBalance')
      .mockImplementation(async () => lightningBalance(rejectTopUp ? 50000 : 150000));
    vi.spyOn(lightningApi, 'getBoardingAddress').mockResolvedValue('bc1qboarding');
    const getSatAmount = vi.spyOn(coinsApi, 'getBtcSatAmount').mockResolvedValue({
      success: true,
      amount: satAmount('100000'),
    });
    ...
    expect(getSatAmount).toHaveBeenLastCalledWith({ amount: '100000', source: 'sat' });
    expect(getBalance).toHaveBeenCalledTimes(2);
    expect(sendTx).not.toHaveBeenCalled();
  });

  it('shows the balance-limit error and disables review inside the top-up page', async () => {
    vi.spyOn(lightningApi, 'getLightningBalance').mockResolvedValue(lightningBalance(0));
