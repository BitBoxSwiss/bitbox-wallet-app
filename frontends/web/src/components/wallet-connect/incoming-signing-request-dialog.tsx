...
export const WCIncomingSignRequestDialog = ({
  ...
  const formattedChain =
    chain in SUPPORTED_CHAINS ? SUPPORTED_CHAINS[chain].name : chain;
  const chainIcon =
    chain in SUPPORTED_CHAINS ? SUPPORTED_CHAINS[chain].icon : null;
  ...
  const textAreaRows =
    signingDataHeight > maxTextAreaRows ? maxTextAreaRows : signingDataHeight;

  return (
    <Dialog open={open} large onClose={onReject}>
      <div className={styles.titleContainer}>
        {isDarkMode ? (
          <WalletConnectLight height={32} width={32} />
        ) : (
          <WalletConnectDark height={40} width={40} />
        )}
            ...
            <ul className={styles.listContainer}>
              <li className={styles.item}>
                <p className={styles.label}>
                  {t('walletConnect.signingRequest.account')}
                </p>
              ...
              <li className={styles.item}>
                <p className={styles.label}>
                  {t('walletConnect.signingRequest.chain')}
                </p>
              ...
              <li className={styles.item}>
                <p className={styles.label}>
                  {t('walletConnect.signingRequest.dapp')}
                </p>
              ...
              {signingData && (
                <li className={styles.item}>
                  <p className={styles.label}>
                    {t('walletConnect.signingRequest.data')}
                  </p>
                  <textarea
                    rows={textAreaRows}
                    className={styles.textarea}
                    readOnly
                    value={signingData.toString()}
                  />
                </li>
              )}
            </ul>

            {stage === 'confirming' && <ConfirmOnBB02 />}

            {stage === 'initial' && (
              <DialogButtons>
                <Button onClick={onAccept} primary type="submit">
                  {t('button.continue')}
                </Button>
                <Button onClick={onReject} secondary type="submit">
                  {t('dialog.cancel')}
                </Button>
              </DialogButtons>
            )}
