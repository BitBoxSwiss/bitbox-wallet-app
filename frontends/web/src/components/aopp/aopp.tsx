...
export const Aopp = () => {
  ...
  useEffect(() => {
    if (aopp !== prevAopp) {
      ...
      if (
        ...
        (prevAopp?.state !== 'choosing-account' ||
          !equal(aopp.accounts, prevAopp?.accounts))
      ) {
  ...
  switch (aopp.state) {
    case 'error':
      return (
        <View fullscreen textCenter verticallyCentered width="580px">
          ...
          <ViewButtons>
            <Button danger onClick={aoppAPI.cancel}>
              {t('button.dismiss')}
            </Button>
          </ViewButtons>
        </View>
    ...
    case 'user-approval':
      ...
      const addressRequestMsg = aopp.xpubRequired
        ? 'aopp.addressRequestWithXPub'
        : 'aopp.addressRequest';
      const addressRequestWithLogoMsg = aopp.xpubRequired
        ? 'aopp.addressRequestWithLogoAndXPub'
        : 'aopp.addressRequestWithLogo';
      return (
        <View fullscreen textCenter verticallyCentered width="580px">
          ...
          <ViewContent>
            ...
            {aopp.xpubRequired ? (
              <Message type="info">
                {' '}
                {t('aopp.xpubRequested', { host: `${host}` })}{' '}
              </Message>
            ) : (
              ''
            )}
    ...
    case 'awaiting-keystore':
      return <Banner>{t('aopp.banner')}</Banner>;
        ...
        <View fullscreen textCenter verticallyCentered width="580px">
          ...
          <ViewContent>
            ...
            <Field>
              ...
              <div className={styles.message}>{aopp.message}</div>
            </Field>
            <PointToBitBox02 />
          </ViewContent>
        </View>
    ...
    case 'success':
      return (
        <View fitContent fullscreen textCenter verticallyCentered width="580px">
          ...
          <ViewButtons>
            <Button primary onClick={aoppAPI.cancel}>
              {t('button.done')}
            </Button>
            <VerifyAddress
