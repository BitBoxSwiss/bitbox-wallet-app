...
// populated. This is the case if there are any non-taproot inputs in the transaction to be signed.
func (p *ProposedTransaction) needsPrevTxs() (bool, error) {
	for _, txIn := range p.TXProposal.Psbt.UnsignedTx.TxIn {
		prevOut, ok := p.TXProposal.PreviousOutputs[txIn.PreviousOutPoint]
		if !ok {
			return false, errp.New("There needs to be exactly one output being spent per input.")
		}
		inputAddress := prevOut.Address
		if inputAddress.AccountConfiguration.ScriptType() != signing.ScriptTypeP2TR {
			return true, nil
		}
	}
	return false, nil
...
// - Previous transactions if any non-taproot inputs are present (PSBT_IN_NON_WITNESS_UTXO)
func (p *ProposedTransaction) Update() error {
	...
	needsPrevTxs, err := p.needsPrevTxs()
	if err != nil {
		return err
	}
	...
	for index, txIn := range txProposal.Psbt.UnsignedTx.TxIn {
		...
		inputAddress := prevOut.Address
		if needsPrevTxs {
			prevTx, err := p.GetPrevTx(txIn.PreviousOutPoint.Hash)
			if err != nil {
				return err
			}
