...
func (sender *outgoingSender) checkFunds(client rpcclient.Interface, tx *types.Transaction) error {
	required := map[*erc20.Token]*big.Int{nil: tx.Cost()}
	record := &ethtypes.TransactionWithMetadata{Transaction: tx}
	if _, amount, ok := record.TokenTransfer(); ok {
		required[erc20.NewToken(tx.To().Hex(), 0)] = amount
	}
...
func (sender *outgoingSender) reconcile(client rpcclient.Interface, height uint64) error {
	...
	finalized := make(map[uint64]common.Hash)
	for hash, record := range sender.records {
		...
		if outgoingTransactionIsFinal(&snapshot, height) {
			finalized[record.Transaction.Nonce()] = hash
		}
	}
	var deleted []common.Hash
	for hash, record := range updated {
		record.NonceConsumed = record.Transaction.Nonce() < nonce
		if winner, exists := finalized[record.Transaction.Nonce()]; exists && winner != hash {
			delete(updated, hash)
