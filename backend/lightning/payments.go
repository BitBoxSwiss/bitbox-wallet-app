...
func (lightning *Lightning) minimumNonDustAmountSat(destinationAddress string) (uint64, error) {
	btcCoin, ok := lightning.btcCoin.(bitcoinAddressToPkScripter)
	if !ok {
		return 0, errp.New("Bitcoin coin does not support address-to-script conversion")
	}
	pkScript, err := btcCoin.AddressToPkScript(destinationAddress)
	if err != nil {
		return 0, errp.WithMessage(errLightningInvalidPaymentInput, err.Error())
	}
	return uint64(mempool.GetDustThreshold(wire.NewTxOut(0, pkScript))), nil
}

func (lightning *Lightning) validateBitcoinPaymentAmount(destinationAddress string, amountSat uint64) error {
	minAmountSat, err := lightning.minimumNonDustAmountSat(destinationAddress)
	if err != nil {
		return err
	}
