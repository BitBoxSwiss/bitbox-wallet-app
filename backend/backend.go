...
func (backend *Backend) initDefaultAccounts() {
	...
	if backend.arguments.Testing() {
		...
		default:
			...
			backend.createAndAddAccount(TETH, "teth", "Ethereum Ropsten BETA", "m/44'/1'/0'/0", signing.ScriptTypeP2WPKH)
			RETH, _ := backend.Coin(coinRETH)
			backend.createAndAddAccount(RETH, "reth", "Ethereum Rinkeby BETA", "m/44'/1'/0'/0", signing.ScriptTypeP2WPKH)
		...
		if backend.arguments.Multisig() {
			...
			backend.createAndAddAccount(ETH, ethAccountCode, "Ethereum BETA", "m/44'/60'/0'/0", signing.ScriptTypeP2WPKH)
