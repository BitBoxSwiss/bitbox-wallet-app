...
type Backend struct {
	...
	makeBtcAccount func(*accounts.AccountConfig, *btc.Coin, *types.GapLimits, func(blockchain.ScriptHashHex) *addresses.AccountAddress, *logrus.Entry) accounts.Interface
...
func NewBackend(arguments *arguments.Arguments, environment Environment) (*Backend, error) {
	...
	backend := &Backend{
		...
		makeBtcAccount: func(config *accounts.AccountConfig, coin *btc.Coin, gapLimits *types.GapLimits, getAddress func(blockchain.ScriptHashHex) *addresses.AccountAddress, log *logrus.Entry) accounts.Interface {
