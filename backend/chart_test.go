...
func TestCalculateMoneyWeightedReturnClampsFutureCashFlowsToEndTime(t *testing.T) {
	startTime := time.Date(2026, time.January, 1, 0, 0, 0, 0, time.UTC)
	endTime := startTime.Add(24 * time.Hour)
	...
	for _, test := range []struct {
		...
		endingValue    float64
		cashFlowValue  float64
	}{
		{name: "deposit", beginningValue: 100, endingValue: 200, cashFlowValue: 100},
		{name: "withdrawal", beginningValue: 200, endingValue: 100, cashFlowValue: -100},
	} {
		t.Run(test.name, func(t *testing.T) {
			result := calculateMoneyWeightedReturn(
				test.beginningValue,
				test.endingValue,
				startTime,
				endTime,
