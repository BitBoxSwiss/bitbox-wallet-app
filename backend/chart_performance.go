...
) (bracket chartLogReturnBracket, found bool) {
	const (
...
// degenerate bracket, or narrows a sign-changing bracket to a root. The second
// return value is false if a non-finite residual prevents bisection.
func bisectChartMoneyWeightedReturn(
	residualAt func(logReturn float64) float64,
	bracket chartLogReturnBracket,
) (logReturn float64, residualsFinite bool) {
...
func solveChartMoneyWeightedReturn(
	...
	bracket, bracketFound := findChartMoneyWeightedReturnBracket(residualAt, residualAtZero)
	if !bracketFound {
		return nil
	}

	logReturn, residualsFinite := bisectChartMoneyWeightedReturn(residualAt, bracket)
	if !residualsFinite {
		return nil
	}
...
func calculateMoneyWeightedReturn(
	...
	for _, cashFlow := range cashFlows {
		if !cashFlow.Time.After(startTime) {
			continue
		}
		...
		cashFlowTime := cashFlow.Time
		if cashFlowTime.After(endTime) {
			// The confirmed balance already includes transactions whose timestamp
			// is ahead of the local clock, so count their cash flow at the endpoint.
			cashFlowTime = endTime
		}
		remainingWeight := endTime.Sub(cashFlowTime).Seconds() / periodSeconds
